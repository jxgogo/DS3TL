...
class Trainer:
    ...
    def train(self, label_loader, unlab_loader, test_loader):
        ...
        with torch.enable_grad():
            batch_idx = 0
            for (label_x, label_y), (targ_x, _), ((unlab_x1, unlab_x2), unlab_y) in zip(cycle(label_loader), cycle(test_loader), unlab_loader):
                batch_idx += 1
                self.global_step += 1
                ...
                with torch.no_grad():
                    ema_outputs = self.ema_modelC(self.ema_modelF(unlab_x2))
                    ema_outputs = ema_outputs.detach()
                cons_loss = self.usp_loss(unlab_outputs, ema_outputs)
